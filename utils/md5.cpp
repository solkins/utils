...
void md5::write(const void* buf, int len)
{
    ...
    if ((count[0] += (inputLen << 3)) < ((unsigned int) inputLen << 3))
        count[1]++;
    ...
    if (inputLen >= partLen)
    {
        memcpy((unsigned char*) & buffer[index], input, partLen);
